        ...
        <col class="col7">
            <?php
                ...
                $query_breakdown = "SELECT
                    ...
                    MAX(p.CREATIONDATETIME) AS TANGGAL,
                ...
                WHERE
                    ...
                    AND p.COUNTERCODE IN ('PBD004', 'PBD001')
                    AND p.STATUS = 3
                GROUP BY
                    p.PMBOMCODE,
                    p2.LONGDESCRIPTION,
            ...
            <thead>
            <tr class="row0">
                <td class="column0 style5 null style9" colspan="2" rowspan="2"><img src="<?php echo base_url('assets/images/logoitti.png'); ?>" width="65" height="60"></td>
...
<?php
    ...
    $no = 1;
    while ($row_breakdown = db2_fetch_assoc($q_breakdown)) {
        $INTERNAL_DOCUMENT = [];
        $WORKORDER_CODE    = $row_breakdown['WORKORDERCODE'];

        $query_internal_document = "SELECT PROVISIONALCODE FROM INTERNALDOCUMENT WHERE PMWORKORDERCODE ='$WORKORDER_CODE'";

        $exec_query_internal_document = db2_exec($conn1, $query_internal_document);

        while ($row_internal = db2_fetch_assoc($exec_query_internal_document)) {
            $INTERNAL_DOCUMENT[] = $row_internal['PROVISIONALCODE'];
        }

        $internal_document_output = implode(", ", $INTERNAL_DOCUMENT);

    ?>
                <tr class="row7">
                    <td class="column0 style25 null"><?php echo $no++; ?></td>
<td class="column1 style26 null" style="text-align: center;">
    <?php echo date("d-M-Y", strtotime($row_breakdown['TANGGAL'])); ?>
</td>
<td class="column2 style27 null style28" colspan="5" style="padding-left: 10px;">
    <?php
        echo $row_breakdown['KEGIATAN'];
            if (! empty($row_breakdown['KEGIATAN']) && ! empty($internal_document_output)) {
                echo " - ";
            }
            echo $internal_document_output;
            // echo $row_breakdown['COUNTERCODE'];
        ?>
</td>
                </tr>
            <?php }?>
